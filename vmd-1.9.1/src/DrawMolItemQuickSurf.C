 ...
 *	$Revision: 1.21 $	$Date: 2011/12/05 05:54:28 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
#if 1
  // XXX reuse existing objects..
  qsurf.calc_surf(atomSel, mol, pos, radii, quality, radscale, 
                  gridspacing, isovalue, colidx, cmap, cmdList);
#else
  // XXX recreate objects every time
  QuickSurf *qs = new QuickSurf();
  qs->calc_surf(atomSel, mol, pos, radii, quality, radscale, 
                gridspacing, isovalue, colidx, cmap, cmdList);
  delete qs; 
#endif
}
