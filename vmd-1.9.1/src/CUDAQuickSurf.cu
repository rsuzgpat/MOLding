 ...
 *      $Revision: 1.43 $      $Date: 2011/12/05 22:13:04 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
__global__ static void gaussdensity_direct(int natoms,
                                    const float4 *xyzr, 
                                    float gridspacing, unsigned int z, 
                                    float *densitygrid) {
  unsigned int xindex  = (blockIdx.x * blockDim.x) * DUNROLLX + threadIdx.x;
...
int vmd_cuda_gaussdensity_direct(long int natoms, float4 *xyzr, float4 *colors,
...
// printf("k[%d/%d] run: %d Gsz.z: %d\n", k, tile.end, runplanes, Gsz.z);
      ...
      if (devtexmap != NULL) {
        ...
        gaussdensity_direct<<<Gsz, Bsz, 0>>>(natoms, 
                                      (float4*) devatoms,
  ...
  gorigin[0]+bbox[0], gorigin[1]+bbox[1], gorigin[2]+bbox[2]);
    ...
    if (chunknumverts > 0) {
      ...
      if (colorperatom) {
        ...
        cmdTriMesh.cuda_putdata(gpuh->v3f_d, gpuh->n3f_d, gpuh->c3f_d,
                                chunknumverts/3, cmdList);
      } else {
        // emit triangle mesh with no colors, uses current rendering state
        cmdTriMesh.cuda_putdata(gpuh->v3f_d, gpuh->n3f_d, NULL,
                                chunknumverts/3, cmdList);
      }
    }
