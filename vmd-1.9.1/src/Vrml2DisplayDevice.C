 ...
 *	$Revision: 1.37 $	$Date: 2011/11/10 17:05:35 $
 *
 ***************************************************************************
 * DESCRIPTION:
    ...
    while (!hersheyDrawNextLine(&hh, &draw, &x, &y)) {
      ...
      if (draw) {
        ...
        if (odraw) {
          ...
          fprintf(outfile, "    color Color { color [ %g %g %g, %g %g %g ] }\n",
...
void Vrml2DisplayDevice::point(float * xyz) {
  ...
  float col[3];
  vec_copy(col, matData[colorIndex]);
  fprintf(outfile, "    color Color { color [%g %g %g] }\n",
          col[0], col[1], col[2]);
  fprintf(outfile, "  }\n");
  fprintf(outfile, "}\n");
}
...
void Vrml2DisplayDevice::tricolor(const float * a, const float * b, const float * c, 
  ...
  fprintf(outfile, "    color Color { color [ %g %g %g, %g %g %g, %g %g %g ] }\n", 
