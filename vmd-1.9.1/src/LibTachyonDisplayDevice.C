...
*      $Revision: 1.84 $        $Date: 2011/03/02 18:32:57 $
*
***************************************************************************
* DESCRIPTION:
*
...
LibTachyonDisplayDevice::LibTachyonDisplayDevice() : FileRenderer ("TachyonInternal", "Tachyon (internal, in-memory rendering)", "vmdscene.tga", DEF_VMDIMAGEVIEWER) { 
...
void LibTachyonDisplayDevice::reset_vars(void) {
  enablemirrorspecular = (getenv("VMDTACHYONMIRRORSPECULAR") != NULL);
  phongspecularthresh = 256.0f;
  if (getenv("VMDTACHYONPHONGSPECTHRESH")) {
    sscanf(getenv("VMDTACHYONPHONGSPECTHRESH"), "%f", &phongspecularthresh);
  }
...
void * LibTachyonDisplayDevice::tex_colormaterial(float *rgb, int material) {
  ...
  if (materials_on) {
    ...
    if (enablemirrorspecular && (mat_shininess >= phongspecularthresh))
      tex.specular=mat_specular;
    else
      tex.specular = 0.0;
  } else {
    tex.ambient  = 1.0;
